...
<div class="max-w-4xl mx-auto space-y-6 animate-fade-in">
    <div class="flex items-center gap-3">
        <a href="{{ route('kabiro.disposisi.index') }}" class="btn-icon" title="Kembali">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Review Disposisi</h1>
    ...
    @if($pengajuan->status->value === 'approved_kabag')
    <div class="glass-card">
        <h3 class="text-lg font-bold text-gray-800 mb-4">Keputusan Disposisi</h3>
        <form id="disposisiForm" method="POST" class="space-y-4">
            @csrf
            <div class="form-group">
                <label class="form-label">Catatan (WAJIB DIISI!)</label>
                <textarea name="notes" class="glass-input" rows="3" placeholder="Tambahkan catatan disposisi..."></textarea>
            </div>
            <div class="flex justify-end gap-3">
                <button type="button" onclick="submitAction('reject')" class="btn-danger">Tolak</button>
                <button type="button" onclick="submitAction('approve')" class="btn-success">Disposisi</button>
            </div>
        </form>
    </div>
    @else
...
function submitAction(action) {
    ...
    if (action === 'reject') {
        form.action = '{{ route("kabiro.disposisi.reject", $pengajuan) }}';
    } else {
        form.action = '{{ route("kabiro.disposisi.approve", $pengajuan) }}';
    }
    form.submit();
}
</script>
