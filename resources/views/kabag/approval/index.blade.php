@extends('layouts.app')
@section('title', 'Approval Kepala Bagian')

@section('content')
<div class="space-y-6 animate-fade-in">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Approval Kepala Bagian</h1>
        <p class="text-gray-500 text-sm mt-1">Kelola persetujuan pengajuan servis kendaraan</p>
    </div>

    <nav class="tab-nav">
        <a href="{{ route('kabag.approval.index') }}"
            class="{{ request()->routeIs('kabag.approval.index') ? 'tab-item-active' : 'tab-item-default' }}">
            Menunggu
            @php $pendingCount = \App\Models\PengajuanServis::where('status', \App\Enums\PengajuanStatusEnum::SUBMITTED)->count(); @endphp
            @if($pendingCount > 0)
        ...
        <a href="{{ route('kabag.approval.history') }}"
            class="{{ request()->routeIs('kabag.approval.history') ? 'tab-item-active' : 'tab-item-default' }}">
            Riwayat
        </a>
    </nav>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                    ...
                    <td class="py-3 px-4 text-center">
                        <a href="{{ route('kabag.approval.show', $item) }}" class="btn-icon" title="Detail">
