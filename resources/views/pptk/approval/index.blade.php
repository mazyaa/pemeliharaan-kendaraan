@extends('layouts.app')
@section('title', 'Approval PPTK')

@section('content')
<div class="space-y-6 animate-fade-in">
    <div>
        <h1 class="text-2xl font-bold text-white">Approval PPTK</h1>
        <p class="text-gray-400 text-sm mt-1">Pengajuan yang menunggu persetujuan PPTK</p>
    </div>

    <nav class="tab-nav">
        <a href="{{ route('pptk.approval.index') }}"
            class="{{ request()->routeIs('pptk.approval.index') ? 'tab-item-active' : 'tab-item-default' }}">
            Menunggu
            @php $pendingCount = \App\Models\PengajuanServis::where('status', \App\Enums\PengajuanStatusEnum::DISPOSED_BIRO)->count(); @endphp
            @if($pendingCount > 0)
        ...
        <a href="{{ route('pptk.approval.history') }}"
            class="{{ request()->routeIs('pptk.approval.history') ? 'tab-item-active' : 'tab-item-default' }}">
            Riwayat
        </a>
    </nav>
            ...
            <tbody>
                ...
                <tr class="border-b border-white/5 hover:bg-white/5 transition-colors">
                    ...
                    <td class="py-3 px-4 text-center">
                        <a href="{{ route('pptk.approval.show', $item) }}" class="btn-icon" title="Detail">
                ...
                <tr>
                    <td colspan="6">
                        <div class="empty-state">
                            ...
                            <p class="empty-text">Tidak ada pengajuan yang menunggu approval</p>
                        </div>
                    </td>
                </tr>
                @endforelse
